...
var TIER_MAP = {
    unless: ['negate_sufficient'],
    none: ['negate_necessary'],
    combined: ['combined_negations']
};

fetch('vedamark-complex-conditionals-bank.json')
...
function pickFromTier(tierKey, n) {
    var catIds = TIER_MAP[tierKey];
    var picked = [];
    catIds.forEach(function(id) {
        if (BANK[id] && BANK[id].length > 0) {
            var shuffled = shuffle(BANK[id]);
...
function startQuiz() {
    ...
    if (!emailUnlocked) { showEmailModal(); return; }
    userAns = {};
    var tierKeys = ['unless', 'none', 'combined'];
    ...
    var tierNames = ['I: Unless / Without / Except', 'II: None / Cannot', 'III: Combined Negations'];
...
function updateLocks() {
    ...
    var locks = document.querySelectorAll('.tier-lock');
    if (emailUnlocked) {
        cards.forEach(function(c) { c.classList.remove('locked') });
        locks.forEach(function(l) { l.textContent = '✓ Unlocked'; l.style.color = '#3A8A5C' });
    } else {
        cards.forEach(function(c) { c.classList.add('locked') });
        locks.forEach(function(l) { l.textContent = '🔒 Email required' });
    }
}
